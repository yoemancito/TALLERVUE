...
const lista = ref([
  ...
  { id: 25, n: 'Pie de Limón', p: 11000, c: 'postres', d: 'Equilibrio crítico con merengue suizo.', img: 'https://images.unsplash.com/photo-1464306076886-da185f6a9d05?w=400' },
...
const agregarProductoMenu = () => {
  ...
  lista.value.push({
    ...
    img: nuevoProducto.value.img.trim() || 'https://images.unsplash.com/photo-1563805042-7684c019e1cb?w=400'
  });
...
const confirmar = () => {
  ...
  carrito.value.forEach((item, index) => {
    const subtotalItem = item.p * item.cantidad
    doc.text(`${index + 1}. ${item.n} x${item.cantidad} - $${subtotalItem.toLocaleString()}`, 20, y)
    y += 10
  })
      ...
      <aside class="cart-sidebar" :class="{ 'movil-abierto': mostrarCarritoMovil }">
        ...
        <div class="cart-box">
          ...
          <div class="cart-list">
            ...
            >
              <div class="item-info">
                <strong>{{ item.n }} <span class="qty-badge">x{{ item.cantidad }}</span></strong>
                <p>${{ (item.p * item.cantidad).toLocaleString() }}</p>
              </div>
      ...
      <section class="menu-display">
        ...
        <div style="background: white; padding: 20px; border-radius: 15px; margin-bottom: 25px; border: 1px solid #eee;">
          ...
          <form @submit.prevent="agregarProductoMenu" style="display: flex; flex-direction: column; gap: 10px;">
            ...
            <input v-model="nuevoProducto.img" type="url" placeholder="URL de la imagen (ej: https://links.com/foto.jpg)" style="padding: 8px; border-radius: 6px; border: 1px solid #ccc;" />
        ...
        <div class="order-details-box">
          <div class="order-scroll">
            <div
              ...
              class="summary-line"
            >
              <span>{{ i.n }} <b>x{{ i.cantidad }}</b></span>
              <span>${{ (i.p * i.cantidad).toLocaleString() }}</span>
            </div>
          </div>
